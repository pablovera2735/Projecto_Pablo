...
describe('MovieListComponent', () => {
  ...
  it('should load genres and movies by genre on init', () => {
    ...
    const req = httpMock.expectOne('http://127.0.0.1:8000/api/movies/genres');
    expect(req.request.method).toBe('GET');
    req.flush(mockGenres);

    const genreReq1 = httpMock.expectOne('http://127.0.0.1:8000/api/movies/genre/1?page=1');
    genreReq1.flush({ movies: [{ id: 101, title: 'Action Movie' }] });

    const genreReq2 = httpMock.expectOne('http://127.0.0.1:8000/api/movies/genre/2?page=1');
