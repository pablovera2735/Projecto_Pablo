...
export class MovieForumComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http.get('http://filmania.ddns.net:8000/api/user', {
  ...
  loadMovieDetails(id: string) {
    this.http.get(`http://filmania.ddns.net:8000/api/movies/${id}`).subscribe((res: any) => {
      this.movie = res.movie_details;
    });
  }

  loadThread(movieId: string) {
    this.http.get(`http://filmania.ddns.net:8000/api/threads/${movieId}`).subscribe((res: any) => {
  ...
  postComment() {
    ...
    this.http.post(
      'http://filmania.ddns.net:8000/api/comments',
      {
